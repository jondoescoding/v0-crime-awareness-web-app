...
- **Format:** Markdown (`.md`)
- **Location:** `/tasks/[within the appropriate folder]`
...
- Tests should be written first
  -  then write the least amount of code possible to make them pass.
- Unit tests should typically be placed alongside the code files they are testing (e.g., `MyComponent.tsx` and `MyComponent.test.tsx` in the same directory).
- Use `npx jest [optional/path/to/test/file]` to run tests. Running without a path executes all tests found by the Jest configuration.

## Interaction Model

The process explicitly requires a pause after generating parent tasks to get user confirmation ("Go", "Yes", or some variation of confirms) before proceeding to generate the detailed sub-tasks. This ensures the high-level plan aligns with user expectations before diving into details.

## Target Audience

Assume the primary reader of the task list is a new **junior developer** who will implement the feature with awareness of the existing codebase context.

## How the task file should look 

At the top of the file:
```
---
name: [brief name of task from the original project file]
...
Each task:
# 1.0 Parent Task [descriptive naming which captures what this is in its entirity]

## Description
[what the task is solving]
[should reference other tasks and the PRD it came from]

## Data Structure (optional)
use this is if we are building features so I can have a clear picture of what is needed to get done:
INPUT
...
## Sub tasks
- [ ] 1.1 [sub task]
  - [Sub-task description 1.1]
- [ ] 1.2 [sub task] 
  - [Sub-task description 1.2]